...
const MrSalaryPrediction = () => {
  ...
  useEffect(() => {
    ...
    if (regression && age !== "" && experience !== "") {
      const income =
        ...
        regression.slopes[0] * age +
        regression.slopes[1] * experience;
  ...
  const regressionPlane = useMemo(() => {
    if (regression) {
      ...
      const x2Range = [
        Math.min(...allExperiences),
        Math.max(...allExperiences),
      ];
      ...
      for (let x1 = x1Range[0]; x1 < x1Range[1]; x1 += x1Step) {
        for (let x2 = x2Range[0]; x2 < x2Range[1]; x2 += x2Step) {
          ...
          const y =
            regression.intercept +
            regression.slopes[0] * x1 +
            regression.slopes[1] * x2;
          yPoints.push(y);
        }
      }
