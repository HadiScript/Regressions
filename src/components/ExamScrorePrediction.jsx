...
const meanStudyHours =
  studyHoursData.reduce((sum, val) => sum + val, 0) / studyHoursData.length;
const meanExamScores =
  examScoresData.reduce((sum, val) => sum + val, 0) / examScoresData.length;

const ExamScorePrediction = () => {
  ...
  useEffect(() => {
    if (inputHours === "") {
      ...
      const score =
        regressionParams.b0 + regressionParams.b1 * parseFloat(inputHours);
  ...
  useEffect(() => {
    if (regressionParams.b0 > 0 && regressionParams.b1 > 0) {
      // Model Testing
      const predictionsFromInputs = studyHoursData.map(
        (x) => regressionParams.b0 + regressionParams.b1 * x
      );
      const residuals = predictionsFromInputs.map(
        (y, i) => examScoresData[i] - y
      );

      console.log(residuals);
      const ssResiduals = residuals.reduce(
        (sum, residual) => sum + Math.pow(residual, 2),
        0
      );
      const ssTotal = examScoresData.reduce(
        (sum, score) => sum + Math.pow(score - meanExamScores, 2),
        0
      );
      ...
      const mae =
        ...
        residuals.length;
      const mse =
        residuals.reduce((sum, residual) => sum + Math.pow(residual, 2), 0) /
        residuals.length;
  ...
  const trainModel = () => {
    ...
    const numerator = studyHoursData.reduce(
      (sum, hour, i) =>
        sum + (hour - meanStudyHours) * (examScoresData[i] - meanExamScores),
      0
    );
    const denominator = studyHoursData.reduce(
      (sum, hour) => sum + Math.pow(hour - meanStudyHours, 2),
      0
    );
  ...
  return (
    <div className="">
      ...
      />
      {predictedScore && <div>Predicted exam score: {predictedScore}</div>}
